      ...
      <div className={s.container}>
        ...
        <div className={s.aboutHero}>
          ...
          <div>
            ...
            {config?.about_text ? (
              <p style={{ fontSize: 15, color: "#6b6560", lineHeight: 1.8, whiteSpace: "pre-line" }}>
                {config.about_text}
              </p>
            ) : (
              <>
        ...
        <div className={`${s.section} ${s.sectionSubtle}`} style={{ borderRadius: 24, padding: "60px 60px" }}>
          ...
          <div className={s.aboutValues}>
            {values.map((v: typeof DEFAULT_ABOUT_VALUES[number], i: number) => (
              <div key={v.title ?? i} className={s.aboutValue}>
        ...
        <section className={s.sectionTight}>
          ...
          <div className={s.grid4} style={{ gap: 24 }}>
            {[
              ...
              { icon: "📍", title: "Studio", text: config?.business_address || "14 Linking Road, Bandra West\nMumbai, 400050" },
